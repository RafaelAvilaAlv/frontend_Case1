  ...
  login(credenciales: any): Observable<any> {
    return this.http.post('http://localhost:8080/api/auth/login', credenciales).pipe(
      tap((respuesta: any) => {
        ...
        const rolOriginal = decoded.rol; // ← debe venir como "ROLE_ADMIN" o "ROLE_USUARIO"

        if (typeof window !== 'undefined') {
          localStorage.setItem('token', respuesta.token);
          localStorage.setItem('rol', rolOriginal); // ← Guardamos completo
        }

        this.tokenSubject.next(respuesta.token);
        this.rolSubject.next(rolOriginal);
      })
    );
  }
